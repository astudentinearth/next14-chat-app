...
export const channelsTable = pgTable("channel", {
    id: text("id").primaryKey(),
    owner: text("owner_id").notNull().references(()=>userTable.id),
    participants: text("participant_ids").references(()=>userTable.id).array(),
    name: varchar("name", {length: 120}),
    creationTime: timestamp("created", {withTimezone: true, mode: "date"}).notNull(),
